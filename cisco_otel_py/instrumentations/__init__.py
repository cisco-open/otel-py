...
class BaseInstrumentorWrapper:
    ...
    def set_payload_max_size(self, max_payload_size) -> None:
        logger.debug('Setting self.max_payload_size to %s.', max_payload_size)
    ...
    def generic_rpc_response_handler(self,
        ...
        try:
            ...
            # Add rpc metadata
            logger.debug('Add Response Headers:')
    ...
    def check_body_size(self, body: str) -> bool:
        ...
        if self.max_payload_size and body_len > self.max_payload_size:
            logger.debug(f"Truncating body to {self.max_payload_size} length")
            return True
        return False
